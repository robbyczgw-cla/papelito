...
Papelito reads Austrian Kindergarten paper, opens a case, and reminds the parent in English two days before the deadline. The first post explains why that requires a case file and a watchdog rather than a translation app. This post covers the implementation, with code from the repository at https://github.com/robbyczgw-cla/papelito, which is private until submission and will be released under MIT at that point.

## One orchestrator, fourteen tools

Papelito is built with Strands Agents, version 1.54.0 at the time of writing. The top-level workflow has one orchestrating `Agent`, a list of fourteen plain Python functions decorated with `@tool`, and a system prompt that tells the model the order to call them in for a new photo. The agent loop picks the next call. Several tools and the watchdog create short-lived specialist agents: the vision reader, an ambiguous-case matcher, the reply and explanation passes, the office lookup, and the optional reminder rewriter.
...
flowchart TD
    ...
    G -- ok --> T5[match_case]
    G -- low --> Q[show the source line, ask one question] --> T5
...
**read_note** is a short-lived Strands agent with the vision model and a system prompt that says one line per printed line, keep umlauts and amounts verbatim, write `[unleserlich]` ("unreadable") for anything unreadable, no commentary. Its model configuration allows up to 2,500 output tokens.

**extract_actions** is where the model proposes and Python verifies. Each action comes back with a kind, a deadline, an amount, the German source line and a confidence. Deterministic checks then look the source line up in the transcription, run the date parser, and regex the amount. A failed check lowers the confidence. At 0.75 or above the artifact is created. From 0.4 up to 0.75 the card shows the source line and asks one question. Below 0.4 nothing is created. The thresholds are at the top of `papelito/extract.py`.
...
**draft_reply** writes a formal German answer using Sie. The register comes from the sender type: warm for a Kindergarten, Hort (after-school care) or Elternverein (parents' association), letter form with a subject line for a Gemeinde (municipality) or Magistrat (municipal authority), a short confirmation for a doctor. Names come from approved profile fields, never from the note. An optional fluency pass checks that numbers and the formal register survive and rejects larger losses of named terms. The tool skips when the paper only informs.
...
**glossary_at** is a static, cited list of Austrian terms. **lookup_office** is the specialist agent for public office information. It applies identifier filtering to the query, then runs over the Web Search Plus MCP server. `MCPClient` attaches it over stdio with only `web_search` and `web_extract` allowed. That MCP server predates the hackathon and is disclosed in the README. Everything else is new.
...
The text model is configured with `reasoning_effort` set to `none` for the structured text rounds. The setting can be overridden for experiments, but the public workflow uses the low-latency configuration.

## The watchdog does not need a model

`papelito watch` runs daily from a systemd user timer. It lists open cases, finds those with a deadline inside two days and no reply marked sent, composes the reminder line and marks the case due. After the earliest active deadline passes, it asks once whether the reply was sent and closes the case. The default line is deterministic, so the timer works with no model key configured. An optional `PAPELITO_WATCH_AGENT=1` mode asks a Strands agent to reword the line, then checks that every digit from the original survived.
